...
# -----------------------------
# Agentic AI Endpoint: Section-wise Resume Improvement
# -----------------------------
def make_improvement_prompt(parsed_resume, job_desc=""):
    return f"""
You are an ATS resume rewriting assistant.
The user provides a parsed resume JSON along with a job description.
Your job is to IMPROVE each section of the resume (skills, experience, summary, education, certifications)
to make it more ATS-friendly, impactful, and professional.
Do NOT invent fake information; only reword and improve clarity, metrics, and keywords.

Return JSON with section-wise improvements:
- sections: {{
    "summary": {{"original": "...", "improved": "..."}},
    "skills": {{"original": [...], "improved": [...]}},
    "experience": [{{"original": "...", "improved": "..."}}],
    "education": {{"original": [...], "improved": [...]}},
    "certifications": {{"original": [...], "improved": [...]}},
}}
- ats_score (number)
- suggestions (list of strings explaining improvements)
...
async def improve_resume(parsed_resume: str = Form(...), job_description: str = Form("")):
    try:
        resume_data = json.loads(parsed_resume)
    except Exception as e:
        return JSONResponse(content={"ok": False, "error": f"Invalid resume JSON: {str(e)}"})

    prompt = make_improvement_prompt(resume_data, job_description)
    response = model.generate_content(prompt)
    raw_text = response.text
    match = re.search(r"\{.*\}", raw_text, flags=re.DOTALL)
    json_text = match.group(0) if match else raw_text.strip()
    ...
    try:
        ...
        improved = {"sections": {}, "ats_score": resume_data.get("ats_score", 0), "suggestions": ["Could not parse JSON, showing raw output"]}

    return JSONResponse(content={"ok": True, "data": improved})
